...
{
    public sealed class PostIdTest
    {
        [Fact]
        public void Create_Nullテスト()
        {
            var ex = Assert.Throws<MultiReasonException>(() => PostId.Create(null!));
        ...
        public void Create_境界テスト_正常(int valueLength)
        {
            ...
            var valueObject = PostId.Create(value);
        ...
        {
            string value = new string('a', valueLength);
            var ex = Assert.Throws<MultiReasonException>(() => PostId.Create(value));
        ...
        {
            string value = new string('a', valueLength);
            var valueObject = PostId.Reconstruct(value);
            Assert.Equal(value, valueObject.Value);
        }
        ...
        {
            var valueObject = PostId.Reconstruct(null!);

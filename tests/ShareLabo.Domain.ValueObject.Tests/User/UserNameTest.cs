...
namespace ShareLabo.Domain.ValueObject.Tests
{
    public sealed class UserNameTest
    {
        ...
        {
            var ex = Assert.Throws<MultiReasonException>(() => UserName.Create(null!));
        ...
        public void Create_境界テスト_正常(int valueLength)
        {
            ...
            var valueObject = UserName.Create(value);
        ...
        {
            string value = new string('a', valueLength);
            var ex = Assert.Throws<MultiReasonException>(() => UserName.Create(value));
        ...
        {
            string value = new string('a', valueLength);
            var valueObject = UserName.Reconstruct(value);
            Assert.Equal(value, valueObject.Value);
        }
        ...
        {
            var valueObject = UserName.Reconstruct(null!);

...
{
    public sealed class TimeLineIdTest
    {
        [Fact]
        public void Create_Nullテスト()
        {
            var ex = Assert.Throws<MultiReasonException>(() => TimeLineId.Create(null!));
        ...
        public void Create_境界テスト_正常(int valueLength)
        {
            ...
            var valueObject = TimeLineId.Create(value);
        ...
        {
            string value = new string('a', valueLength);
            var ex = Assert.Throws<MultiReasonException>(() => TimeLineId.Create(value));
        ...
        {
            string value = new string('a', valueLength);
            var valueObject = TimeLineId.Reconstruct(value);
            Assert.Equal(value, valueObject.Value);
        }
        ...
        {
            var valueObject = TimeLineId.Reconstruct(null!);

...
{
    public sealed class TimeLineNameTest
    {
        [Fact]
        public void Create_Nullテスト()
        {
            var ex = Assert.Throws<MultiReasonException>(() => TimeLineName.Create(null!));
        ...
        public void Create_境界テスト_正常(int valueLength)
        {
            ...
            var valueObject = TimeLineName.Create(value);
        ...
        {
            string value = new string('a', valueLength);
            var ex = Assert.Throws<MultiReasonException>(() => TimeLineName.Create(value));
        ...
        {
            string value = new string('a', valueLength);
            var valueObject = TimeLineName.Reconstruct(value);
            Assert.Equal(value, valueObject.Value);
        }
        ...
        {
            var valueObject = TimeLineName.Reconstruct(null!);

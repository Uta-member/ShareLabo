...
{
    public sealed class UserDetailFindByOAuthIdentifierQueryService : IUserDetailFindByOAuthIdentifierQueryService
    {
        private readonly ShareLaboPGSQLConnectionFactory _connectionFactory;

        public UserDetailFindByOAuthIdentifierQueryService(ShareLaboPGSQLConnectionFactory connectionFactory)
        {
            _connectionFactory = connectionFactory;
        }

        public async ValueTask<IUserDetailFindByOAuthIdentifierQueryService.Res> ExecuteAsync(
            IUserDetailFindByOAuthIdentifierQueryService.Req req,
            CancellationToken cancellationToken = default)
        {
            using var connection = _connectionFactory.OpenConnection();
            var factory = new QueryFactory(connection, new PostgresCompiler());

            var dbOAuthIntegration = await factory.Query("o_auth_integrations")
                .Where("o_auth_type", req.OAuthType)
                .Where("o_auth_identifier", req.OAuthIdentifier)
                .FirstOrDefaultAsync<DbOAuthIntegration>();

            if(dbOAuthIntegration is null)
            {
            ...
            var query = factory.Query("users as user_info")
                .Where("user_info.user_id", dbOAuthIntegration.UserId)
